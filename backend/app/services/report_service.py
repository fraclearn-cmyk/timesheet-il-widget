...
class ReportService:
    ...
    def get_daily_report(
        ...
        for session in sessions:
            ...
            activity_count = (
                ...
                .filter(ActivitySession.work_session_id == session.id)
                .count()
            )
        ...
        total_users = len(set(s.amocrm_user_id for s in sessions))
    ...
    def get_monthly_report(
        ...
        while current_date.weekday() != 0 and current_date > month_start - timedelta(
            days=7
        ):
            current_date -= timedelta(days=1)
    ...
    def get_employee_report(
        ...
        activities = (
            ...
            .group_by(ActivitySession.entity_type)
            .all()
        )
        ...
        average_work_time = (
            total_work_time / total_days_worked if total_days_worked > 0 else 0
        )
    ...
    def get_period_statistics(
        ...
        users_data = {}
        for session in sessions:
            if session.amocrm_user_id not in users_data:
                users_data[session.amocrm_user_id] = {
                    "user_name": session.user_name,
                    "sessions": [],
                    "total_work_time": 0,
                    "total_break_time": 0,
            ...
            users_data[session.amocrm_user_id]["sessions"].append(session)
            users_data[session.amocrm_user_id][
                "total_work_time"
