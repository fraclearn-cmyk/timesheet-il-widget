...
class ExcelService:
    ...
    def generate_department_report(
        ...
        for session in sessions:
            ws.cell(
                ...
                column=1,
                value=(
                    session.user.department.name if session.user.department else "N/A"
                ),
            )
            ...
            ws.cell(
                row=row, column=8, value=session.late_minutes if session.is_late else 0
            )

            if include_comments:
                comment = (
                    self.db.query(WorkComment)
                    .filter(WorkComment.work_session_id == session.id)
                    .first()
                )
    ...
    def generate_employee_report(
        ...
        # Header
        ws.cell(row=1, column=1, value=f"Отчёт: {user.name}").font = Font(
            bold=True, size=14
        )
        ...
        for session in sessions:
            ...
            ws.cell(
                row=row, column=6, value=self._format_time(session.total_break_time)
            )
            ws.cell(
                row=row,
                column=7,
                value=f"{session.late_minutes} мин" if session.is_late else "-",
            )
    ...
    def _add_header(self, ws, title: str, date_from: date, date_to: date):
        ...
        ws.cell(
            row=3,
            column=1,
            value=f"Сформирован: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        )
