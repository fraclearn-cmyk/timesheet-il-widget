...
def take_break(user_id: int, db: Session = Depends(get_db)):
    """Take a break"""
    service = SessionService(db)
    try:
...
def resume_work(user_id: int, db: Session = Depends(get_db)):
...
def finish_session(user_id: int, db: Session = Depends(get_db)):
    """Finish work session"""
    service = SessionService(db)
    try:
