...
class WorkSessionBase(BaseModel):
    ...
    user_id: int = Field(
        gt=0,
        validation_alias=AliasChoices("amocrm_user_id", "user_id"),
        description="Deprecated wire alias for external amoCRM user ID, never User.id",
    )
    user_name: str
