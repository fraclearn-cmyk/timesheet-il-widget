...
| Выровнять существующие модели, сервисы и API | Модели, схемы, маршруты сессий, KPI, отчёты, Excel и team-сервис переведены на согласованные поля и account-scoped идентификаторы | Выполнено | Нет; проверка прав доступа остаётся задачей фазы 2 |
...
Фазы 0 и 1 завершены. OAuth/account/users/rights/refresh и controlled analytics spike проверены live; единая модель, сервисы и миграция `005` проверены unit/integration-тестами и независимым review. Следующая по порядку — фаза 2: OAuth, синхронизация пользователей и права доступа.
