...
|---:|---|---|---|---|
| 0 | Контракт и технический spike amoCRM | Выполнена с внешним ограничением | 2026-09-11 | Статическая проверка структуры и контрактов; реальный OAuth spike ждёт тестовый amoCRM |
| 1 | Единая модель данных и миграции | В работе | 2026-09-11 | Модели/граф миграций проверены; `alembic check` требует согласовать Python-интерпретатор |
...
|---|---|---|---|
| Проверить доступность API событий аналитики amoCRM и звонков | Локальный mock-контракт и список неизвестных полей зафиксированы в `docs/`; реальные ответы не получены | Выполнена с ограничением | Нужен тестовый amoCRM-аккаунт и OAuth |
...
- [ ] Составить таблицу требований из этого плана и текущего кода в `docs/requirements-matrix.md`.
- [ ] Проверить manifest и доступные locations/scopes для установленного виджета в `widget/manifest.json`.
...
- [ ] Описать fallback, если API amoCRM не отдаёт отдельное поле или событие, в `docs/amocrm-integration-limits.md`.
- [ ] Зафиксировать JSON-контракты: `GET /me`, `GET /settings`, status transitions, team, timeline, report, export.
...
- Реальный OAuth, полный список аналитических событий и звонки нельзя подтвердить без тестового amoCRM-аккаунта и доступа к нему.
...
Фаза 0 завершена на локальном уровне. Фаза 1 в работе: модельный слой и граф миграций добавлены, но чистое применение миграций и выравнивание старых сервисов ещё не завершены. Текущий внешний блокер для интеграционных фаз — отсутствие подтверждённого тестового OAuth-доступа к аналитическим событиям и звонкам amoCRM.
