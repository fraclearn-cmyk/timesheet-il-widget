...
|---:|---|---|---|---|
| 0 | Контракт и технический spike amoCRM | Заблокирована на live-проверке | 2026-09-11 | 21 mock-контрактный тест проходит; для live spike нужна авторизованная сессия тестового amoCRM |
| 1 | Единая модель данных и миграции | Приостановлена до фазы 0 | 2026-09-11 | Модели/граф миграций проверены; продолжение после live spike фазы 0 |
| 2 | OAuth, синхронизация пользователей и права | Не начата | — | — |
...
|---|---|---|---|
| Проверить доступность API событий аналитики amoCRM и звонков | Добавлен безопасный OAuth/событийный adapter и 21 контрактный тест; реальные ответы ещё не получены | Заблокировано на live-проверке | Тестовый аккаунт есть, но нужна авторизованная браузерная сессия или рабочий OAuth code/token |
| Выровнять существующие модели, сервисы и API | Выявлено и описано в контракте; код ещё не выровнен | Не начато | Текущий код содержит рассинхрон полей и маршрутов |
| Ввести отдельные группы виджета и уникальность группы сотрудника | Добавлены модели `WidgetGroup`/`GroupMember` и уникальность `(account_id, user_id)` | В работе | Нужно проверить upgrade чистой БД |
| Реализовать настройки «Учитывать» и «Скрыть» | Не выполнено | Не начато | Зависит от синхронизации пользователей |
...
| Реализовать интервалы с порогом 5 минут | Добавлена модель `ActivityInterval`; логика ещё не подключена | Не начато | Нужны сервисы активности и подтверждённые CRM-события |
...
- [ ] Проверить, как amoCRM передаёт OAuth authorization code, refresh token, account ID, user ID и роли.
- [ ] Проверить API/аналитику событий: полный список типов, автора, timestamp, сущность, ссылку на карточку, пагинацию и задержку появления.
- [ ] Проверить API звонков: входящий/исходящий тип, длительность, автор и карточка.
- [ ] Проверить, может ли виджет слушать мышь/клавиатуру всей страницы или только своей области; не считать это единственным источником активности.
...
- [ ] Зафиксировать идентификаторы: внутренний `User.id`, `amocrm_user_id`, `amocrm_account_id`; убрать доверие к клиентскому внутреннему ID.
- [ ] Создать/обновить модели `User`, `WidgetSettings`, `WidgetGroup`, `GroupMember`, `WorkSession`, `StatusTransition`, `ActivityInterval`, `CrmEvent`, `CallEvent`, `WorkComment`.
- [ ] Добавить группе `timezone`, `work_start_time`, `work_end_time`, `is_active`.
- [ ] Добавить уникальное ограничение одного активного членства сотрудника в одной группе.
- [ ] Разделить `active_duration`, `unconfirmed_duration`, `break_duration` и `idle_duration`.
- [ ] Добавить источник интервала: `crm_event`, `call`, `unconfirmed_input`; незарегистрированные клики не записывать как CRM activity.
- [ ] Создать последовательную миграцию после текущей цепочки и проверить upgrade/downgrade на копии БД.
- [ ] Выровнять сервисы и схемы с фактическими полями; удалить или переписать обращения к отсутствующим `WorkSession.account_id`, `status`, `session_id`, `WorkComment.session_id`.
...
- [ ] Создать `backend/app/integrations/amocrm_client.py` с timeout, retry для временных ошибок, rate-limit обработкой и безопасным логированием.
- [ ] Создать `backend/app/integrations/oauth.py` для authorization code, refresh token и шифрования/маскировки токенов.
- [ ] Создать `backend/app/services/user_sync_service.py` для users, ролей, имени, email, avatar и account metadata.
- [ ] Создать `backend/app/core/access_policy.py`: employee/manager/admin, проверка видимости группы и сотрудника.
- [ ] Заменить доверенные заголовки `X-User-Id`/`X-Account-Id` на проверенный контекст запроса; оставить заголовки только как тестовый adapter.
- [ ] Синхронизировать изменения состава пользователей без удаления истории табеля.
- [ ] Добавить 401/403/404/409/429 тесты и проверку прямого запроса РОП к чужой группе.
...
- Проведена инвентаризация загруженной amoMarket-оболочки: рабочим источником в репозитории является `widget/`, отдельного `widget.zip` нет.
...
- Добавлен 21 контрактный тест; независимое ревью и два раунда исправлений закрыли найденные ошибки.
...
- Сверить текущий `widget/manifest.json` с описанным ограничением: рабочий архив уже имеет `advanced_settings`, но реальный OAuth и события пока не подключены.
- На тестовом amoCRM-аккаунте выполнить сценарий из `docs/amocrm-integration-limits.md`.
- Локально выполнить `D:\табель\.venv312\Scripts\python.exe -m pytest -q` из `backend/` — ожидается `21 passed`.

#### Блокеры и остаточные риски

- Тестовый аккаунт есть, но текущая автоматизация не видит авторизованную браузерную сессию, а в локальном `.env` OAuth client ID/secret не заполнены рабочими значениями.
- Для live spike нужно открыть тестовый аккаунт в доступном браузере и авторизоваться либо настроить рабочие OAuth-данные локально; секреты не следует отправлять в чат.
- Поэтому следующие фазы используют контрактный слой и не должны выдавать неподтверждённые события за реальные.

### Отчёт по текущему состоянию фазы 1

#### Что сделано

- Добавлены модели `WidgetGroup`, `GroupMember`, `CrmEvent`, `CallEvent`, `ActivityInterval`.
- Для группы добавлены часовой пояс, начало/конец рабочего дня и руководитель.
- Для членства добавлены `track_time` и `hide_widget`; уникальность не позволяет включить пользователя в две группы одного аккаунта.
- Новые модели подключены к `backend/migrations/env.py`.
- Добавлена начальная миграция базовых таблиц и миграция новых сущностей.
- Исправлен граф Alembic: `001 → 002 → 003 → e1db → 004`, одна голова `004`.

#### Созданные и изменённые файлы

- Созданы: `backend/app/models/widget_group.py`, `group_member.py`, `crm_event.py`, `call_event.py`, `activity_interval.py`.
- Изменены: `backend/app/models/__init__.py`, `user.py`, `department.py`, `backend/migrations/env.py`.
- Созданы: `backend/migrations/versions/001_initial_timesheet_tables.py`, `004_widget_groups_activity.py`.
- Изменены: `backend/migrations/versions/002_add_reports_table.py`, `e1db632ded80_add_remaining_tables.py`.

#### Что можно проверить

- `python -m compileall -q backend/app/models backend/migrations` — проходит.
- `alembic heads` из `backend/` — показывает единственную голову `004`.
- Открыть модели и проверить связи группы, членства и интервала.

#### Блокеры и остаточные риски

- Команда `alembic check` запускается через Python 3.12, а установленный драйвер находится в настроенном Python 3.14; нужно запускать Alembic одним и тем же интерпретатором.
- Чистое `upgrade head` ещё не проверено на реально доступной PostgreSQL-базе.
- Существующие сервисы всё ещё используют старые имена полей; их выравнивание входит в оставшуюся часть фазы 1.

## Текущий отчёт по фазам

Локальная часть фазы 0 прошла 21 контрактный тест и независимое ревью, но фаза не закрыта без live spike. Фаза 1 приостановлена: модельный слой и граф миграций добавлены, но продолжение начнётся после авторизации тестового amoCRM и фиксации реальных OAuth/analytics/call контрактов.
