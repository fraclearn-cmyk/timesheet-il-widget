...
| Пагинация и задержка | Не терять повторные/задержанные CRM-события | Users list live содержит `_page`, `_page_count`, `_total_items`; при `limit=1` наблюдались metadata 1/1/1. Events — 204 | Параметры cursor/page и окно задержки не определены до live spike | Users pagination metadata наблюдались; обход нескольких страниц и page behavior не проверены; event latency/dedup нет |
