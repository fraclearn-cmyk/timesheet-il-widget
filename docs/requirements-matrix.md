...
Источник: требования проекта, фактический код, mock-контракты и controlled live spike
от 2026-09-16: начальное read-only чтение CRM с отдельно разрешённым OAuth refresh,
затем разрешённая пользователем controlled-write event generation — создание
`[TEST CODEX]` contact/company/lead/task/common note и PATCH только test lead.
Статус «mock» означает, что поведение проверено изолированным
...
| Активность | Только события amoCRM и звонки | Controlled `[TEST CODEX]` actions дали 11 events через `GET /api/v4/events?limit=250&page=1` — 200 | Normalizer принимает bounded opaque string ID, integer author/timestamp/entity ID и только observed type/entity contexts; `_embedded.entity._links.self.href` — same-tenant API entity URL, не UI-карточка | Ограниченный live field shape/types подтверждён и покрыт синтетическими тестами; полный каталог и UI card routing не известны |
